...
public:
	...
	void Setup(const char * eventName,
		const char * argName,
		const char * argType,
		const char * argRange);
...
private: friend class Listener;
		 ...
		 static int compareEvents(const void * arg1,
			 const void * arg2);
...
public: CLASS_PROTOTYPE(Event);
		...
		friend bool operator == (const Event & a,
			const Event & b);
		friend bool operator != (const Event & a,
			const Event & b);
...
private: void ScriptRemove(Event * e);

protected: qboolean CheckEventFlags(Event * event);

public: CLASS_PROTOTYPE(Listener);
